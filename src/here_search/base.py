...
from .api import (API, PartialTextSearchEvent, SearchEvent, TextSearchEvent, TaxonomySearchEvent, FollowUpSearchEvent, DetailsSearchEvent, EmptySearchEvent,
                  SearchIntent, FormulatedIntent,
                  NoIntent, UnsupportedIntentMaterialization, UnsupportedSearchEvent)
from .entities import (Response, ResponseItem, PlaceTaxonomyItem, SearchContext, AutosuggestConfig,
                       DiscoverConfig, BrowseConfig, LookupConfig)
...
class OneBoxSimple:
    ...
    async def handle_search_event(self, session: ClientSession) -> None:
        ...
        if isinstance(event, PartialTextSearchEvent):
            config = AutosuggestConfig(limit=self.suggestions_limit, terms_limit=self.terms_limit)
    ...
    async def handle_search_response(self, event: SearchEvent, resp: Response, session: ClientSession) -> None:
        if isinstance(event, PartialTextSearchEvent):
    ...
    async def wait_for_search_event(self) -> SearchEvent:
        context = SearchContext(latitude=self.search_center[0], longitude=self.search_center[1], language=self.language)
        intent: SearchIntent = await self.queue.get()
        if isinstance(intent.materialization, str):
            if isinstance(intent, FormulatedIntent):
                return TextSearchEvent(context=context, query_text=intent.materialization)
            else:
                return PartialTextSearchEvent(context=context, query_text=intent.materialization)
            ...
            if intent.materialization.data["resultType"] in ("categoryQuery", "chainQuery"):
    ...
    def handle_suggestion_list(self, response: Response, session: ClientSession) -> None:
    ...
    def run(
        self,
        handle_search_events: Callable = None
    ) -> "OneBoxSimple":
