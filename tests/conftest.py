...
from here_search.api import API, base_url, PartialTextSearchEvent, TextSearchEvent, TaxonomySearchEvent, DetailsSearchEvent, EmptySearchEvent
from here_search.entities import Request, Endpoint, SearchContext, Response, ResponseItem, AutosuggestConfig, DiscoverConfig, BrowseConfig, LookupConfig, PlaceTaxonomyItem
...
def autosuggest_request(context, instant_query_text, x_headers):
    return Request(
        endpoint=Endpoint.AUTOSUGGEST, url=base_url[Endpoint.AUTOSUGGEST],
        params={"q": instant_query_text, "at": f"{context.latitude},{context.longitude}"},
        x_headers=x_headers
    )
...
def response(a_request):
    data = [{"resultType": "place"}, {"resultType": "chainQuery"}, {"resultType": "categoryQuery"}]
