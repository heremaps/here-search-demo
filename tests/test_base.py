...
from here_search.api import (FormulatedIntent, SearchIntent, NoIntent,
                             SearchEvent, PartialTextSearchEvent, TextSearchEvent, TaxonomySearchEvent,
                             FollowUpSearchEvent, DetailsSearchEvent,
                             EmptySearchEvent, UnsupportedIntentMaterialization)
from here_search.entities import (SearchContext, PlaceTaxonomyExample)
...
async def test_wait_for_search_event_1(app, query_text, context):
    ...
    with patch.object(SearchContext, "__call__", return_value=context):
        intent = FormulatedIntent(materialization=query_text)
        app.queue.put_nowait(intent)
        event: SearchEvent = await app.wait_for_search_event()
        assert isinstance(event, TextSearchEvent) and event.query_text == query_text


@pytest.mark.asyncio
async def test_wait_for_search_event_2(app, context):
    """
    Tests the reception of an instant text
    """
    with patch.object(SearchContext, "__call__", return_value=context):
        intent = SearchIntent(materialization="restaurant")
        app.queue.put_nowait(intent)
        event: SearchEvent = await app.wait_for_search_event()
        assert isinstance(event, PartialTextSearchEvent) and event.query_text == "restaurant"
...
async def test_wait_for_search_event_4(app, location_response_item, context):
    ...
    """
    with patch.object(SearchContext, "__call__", return_value=context):
        intent = SearchIntent(materialization=location_response_item)
        app.queue.put_nowait(intent)
        event: SearchEvent = await app.wait_for_search_event()
...
async def test_wait_for_search_event_5(app, chain_query_response_item, context):
    ...
    """
    with patch.object(SearchContext, "__call__", return_value=context):
        intent = SearchIntent(materialization=chain_query_response_item)
        app.queue.put_nowait(intent)
        event: SearchEvent = await app.wait_for_search_event()
...
async def test_wait_for_search_event_6(app, category_query_response_item, context):
    ...
    """
    with patch.object(SearchContext, "__call__", return_value=context):
        intent = SearchIntent(materialization=category_query_response_item)
        app.queue.put_nowait(intent)
        event: SearchEvent = await app.wait_for_search_event()
...
@pytest.mark.asyncio
@pytest.mark.parametrize("event, config", [(pytest.lazy_fixture('partial_text_search_event'),
                                           ...
                                           (pytest.lazy_fixture('empty_search_event'),
                                            None)
                                           ])
async def test_handle_search_event(app, event, config, response, session):
    ...
    """
    with patch.object(app, "wait_for_search_event", return_value=event) as wfse, \
         patch.object(app, "handle_search_response", return_value=None) as hsr, \
         patch.object(event, "get_response", return_value=response) as gr:
...
async def test_handle_search_response_1(app, query_text, context, response, session):
    with patch.object(app, "handle_suggestion_list", return_value=None) as hsl, \
            patch.object(app, "handle_result_list", return_value=None) as hrl, \
            patch.object(app, "handle_empty_text_submission", return_value=None) as hets:
